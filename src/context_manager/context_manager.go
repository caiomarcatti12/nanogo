...
var instance *SafeContextManager
var once sync.Once
...
func (scm *SafeContextManager) SetValues(values map[interface{}]interface{}, handler func()) {
	scm.mu.Lock()
	valuesToSet := gls.Values(values) // Copie os valores enquanto estiver sob bloqueio, se necessário.
	scm.mu.Unlock()

	scm.mgr.SetValues(valuesToSet, handler)
	//scm.mgr.SetValues(gls.Values(values), handler)
}
